...
export default function AlertsCard({ error, onRetry }) {
  ...
  useEffect(() => {
    function check() {
      getSecretStatus("modrinth")
        .then((s) => setTokenMissing(!s.exists))
        .catch(() => setTokenMissing(true));
    }
    check();
    window.addEventListener("modrinth-change", check);
    return () => window.removeEventListener("modrinth-change", check);
  }, []);
  ...
  if (tokenMissing && !dismissed.token) {
    alerts.push({
      ...
      actions: (
        <Link to="/settings">
          <Button variant="outline" className="h-8 px-sm">
            Open Settings
          </Button>
        </Link>
      ),
  ...
  if (error && !dismissed.error) {
    const message =
      error === "rate limited" ? "Rate limit hit." : "Failed to load data.";
    alerts.push({
      key: "error",
      message,
      actions: (
