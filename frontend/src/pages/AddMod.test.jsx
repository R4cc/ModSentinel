...
vi.mock("@/lib/api.ts", () => ({
  getSecretStatus: vi
    .fn()
    .mockResolvedValue({ exists: true, last4: "", updated_at: "" }),
  addMod: vi.fn(),
  getInstance: vi.fn(),
  getMods: vi.fn(),
}));
...
describe("AddMod page", () => {
  ...
  it("prefills slug from unresolved file", async () => {
    render(
      <MemoryRouter
        initialEntries={[
          { pathname: "/instances/1/add", state: { file: "sodium-1.0.jar" } },
        ]}
      >
        <Routes>
          <Route path="/instances/:id/add" element={<AddMod />} />
