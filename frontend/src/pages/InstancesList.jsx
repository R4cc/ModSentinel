...
function openEdit(inst) {
    ...
    setServerError("");
    setOpen(true);
    setTimeout(() => { try { document.getElementById('instance_edit_loader')?.focus(); } catch {} }, 0);
}
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      // Navigate to the instance page. For local instances, wait for user to bind a PufferPanel server before sync.
          ...
          {instances.map((inst) => (
            ...
            >
              <CardHeader className="pb-0">
                <div className="flex items-center justify-between gap-sm">
                  ...
                  <div className="flex items-center gap-xs">
                    {inst.requires_loader && (
                      <Badge variant="secondary" className="border bg-red-50 text-red-700 border-red-200">Set loader manually</Badge>
                    )}
              ...
              <CardFooter className="flex justify-end gap-xs">
                {inst.requires_loader ? (
                  <>
                    <Button size="sm" onClick={(e) => { e.stopPropagation(); openEdit(inst); }} title="Set loader">
                      Set loader
                    </Button>
                    <Button size="sm" variant="outline" onClick={(e) => e.stopPropagation()} disabled title="Set loader first" aria-label="Delete instance">
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </>
                ) : (
                  <>
                    <Button size="sm" onClick={(e) => { e.stopPropagation(); openEdit(inst); }}>
                      Edit
                    </Button>
                    <Button size="sm" variant="outline" onClick={(e) => { e.stopPropagation(); openDelete(inst); }} aria-label="Delete instance">
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </>
                )}
      ...
      <Modal open={open} onClose={() => setOpen(false)}>
        <form className="space-y-md" onSubmit={handleSave}>
          ...
          {editing && (
            <>
              ...
              <div className="space-y-xs">
                ...
                <LoaderSelect id="instance_edit_loader" loaders={loaders} value={loader} onChange={setLoader} disabled={!loadersLoaded || loaders.length === 0} />
              </div>
            </>
          )}
          ...
          <div className="flex justify-end gap-sm">
            ...
            <Button type="submit" disabled={editing ? (!name.trim() || !loader) : (scanning || (addTab === "puffer" ? (loadingServers || !selectedServer) : (!name.trim() || !loadersLoaded || !loader)))} aria-busy={scanning}>
              {editing ? "Save" : "Add"}
            </Button>
          </div>
        </form>
